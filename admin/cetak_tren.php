...
$q_data = query($koneksi, "
    SELECT DATE(tgl_lapor) as tanggal, COUNT(id_tiket) as total 
    FROM tiket 
    $sql_where 
    ...
    while ($row = mysqli_fetch_assoc($q_data)) {
        ...
        $html_rows .= '
            <tr>
                ...
                <td align="center">'.$row['total'].' Tiket</td>
            </tr>';
    }
} else {
    $html_rows = '<tr><td colspan="3" align="center">Data tidak ditemukan pada periode ini.</td></tr>';
}
...
<head>
    <style>
        ...
        .ttd-box { width: 40%; text-align: center; float: right; }
        .ttd-space { height: 70px; }
        .pimpinan-name { font-weight: bold; text-decoration: underline; }
    </style>
</head>
<body>
    ...
    <div class="judul-laporan">LAPORAN TREN GANGGUAN PELANGGAN</div>
    ...
    <table class="tabel-data">
        <thead>
            <tr>
                <th width="8%">NO</th>
                <th width="50%">TANGGAL PELAPORAN</th>
                <th width="42%">JUMLAH TIKET PENGADUAN</th>
            </tr>
        </thead>
        <tbody>
            '.$html_rows.'
...
try {
    ...
    $mpdf->SetTitle('Laporan_Tren_'.date('Ymd'));
    $mpdf->WriteHTML($html);
    $mpdf->Output('Laporan_Tren_Gangguan.pdf', 'I'); 
